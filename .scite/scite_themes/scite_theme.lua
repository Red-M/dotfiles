...
local locate_scheme = function(pdir, name)
	...
	local list = dofile(props['ext.lua.theme_dir']..'/scheme_list.lua')
...
local apply_scheme = function(name)
    ...
    local f = io.open(theme_dir..'/theming.properties', 'w')
		...
		for line in io.lines(template_path) do
			...
			if (line:match('%S') ~= nil) and (line:match('^%s*#') == nil) then
				...
				if key then
                    ...
                    f:write(key.."="..value:gsub('$%(colou?r%s+([%w%p ]+)%)', replacefn).."\n")
...
function load_themes()
	local theme_dir = props['ext.lua.theme_dir']
	local cmd = [[find schemes/ -name '*.json' -o -name '*.yaml'|sort]]
	local gen_scheme_list_lua = function()
		local f = io.open(theme_dir..'/scheme_list.lua', 'w')
